...
class DETRHOI(nn.Module):
    def __init__(
        self, backbone, transformer, num_obj_classes, num_verb_classes, num_queries
    ):
        super().__init__()
    ...
    def forward(self, outputs, target_sizes):
        ...
        for os, ol, vs, sb, ob in zip(
            ...
            bboxes = [
                ...
                for bbox, label in zip(b.to("cpu").numpy(), l.to("cpu").numpy())
            ]

            hoi_scores = vs * os.unsqueeze(1)

            verb_labels = (
                torch.arange(hoi_scores.shape[1], device=self.correct_mat.device)
                .view(1, -1)
...
def main(args):
    ...
    post_processor = PostProcessHOI(
        args.num_queries, args.subject_category_id, dataset_val.correct_mat
    )
...
@torch.no_grad()
def generate(
    model, post_processor, data_loader, device, verb_classes, missing_category_id
):
    model.eval()
